...
> **当前版本：v26.09.33**（版本号以 [`web/VERSION`](web/VERSION) 为唯一来源）。**项目已进入休眠维护阶段：不再新增功能，仅修复缺陷与兼容性问题**。各版本变更见下方「[更新日志](#-更新日志)」。

## 📝 更新日志

> **维护约定**：本文档主体始终描述最新版本的完整功能；每次发布新版本时，在本章节**顶部**追加一条版本记录，按「新增 / 变更 / 修复 / 移除」分类列出改动，同时同步修订正文对应描述。版本号与 [`web/VERSION`](web/VERSION) 文件保持一致，采用 `YY.MM.NN` 序号格式（年.月.当月第 N 次版本迭代，非日期）。v26.09.14 之前的未版本化历史不再追溯。

### v26.09.33

AUTH_KEY 随机生成规则统一，无新增功能。

**变更**

- 🔑 首次启动自动生成的 `AUTH_KEY` 从约 32 位 URL 安全随机字符串（不含符号）改为与网页「🎲 随机」完全一致：14 位，至少 1 个大写、1 个小写、1 个数字、1-3 个符号（仅 `!@#$%^&*`），其余位由字母/数字补足
- 🔑 管理网页「🎲 随机」生成的符号池从 `!@#$%^&*?-_=+` 缩减为 `!@#$%^&*`，生成的 KEY 不再包含 `? - _ = +` 五种符号

### v26.09.32

修复 m3u8 下载进度条解析错误，并新增 m3u8 分片计数显示。

**新增**

- ✨ m3u8 下载任务的进度百分比左侧显示「已下载分片/总分片」（如 `13/30 43%`），任务详情弹窗的「进度」行同步显示；分片计数取自 N_m3u8DL-RE 实时输出，仅 m3u8 下载显示，直链下载、暂停恢复等待中的任务不显示

**修复**

- 🐛 m3u8 下载进度条显示混乱：进度百分比取自 N_m3u8DL-RE 输出行（形如 `13/30 43.33%`），原解析不兼容小数百分比，`43.33%` 被误读为 `33%`、`10.00%` 被误读为 `0%`，导致进度条在 0/33/67 之间反复跳动，看似"每个分片各自的进度"而非整体下载进度；现正确解析小数百分比，进度条平滑反映整个视频的真实下载进度，且已开始下载但进度不足 1% 时至少显示 1%，避免被误认为未在下载

### v26.09.31

修复日志级别与下载日志可见性，无新增功能。

**修复**

- 🐛 关闭调试模式时 `debug_print` 仍向日志文件写入 DEBUG 级别内容，导致「日志导出」也带 DEBUG 行；现改为 debug 关闭时既不输出控制台也不写文件，仅开启 debug 时才记录

**变更**

- 📋 下载相关日志（开始下载 / 下载完成 / 下载失败 / 跳过下载 / 已暂停 / 已删除 / 恢复下载，共 16 处）统一加 `[用户名]` 前缀，格式 `[user] 操作 文件名`，便于从日志追溯谁在下载什么
...
- 🧹 启动成功时不再输出冗余调试信息（系统配置摘要、过滤规则加载统计、AUTH_KEY 回写路径）；启动失败（`URL_PREFIX` 未设置）改为将失败原因写入日志文件，便于排查
- 🖥️ 启动横幅（banner）三条分区标题行（猫抓插件配置 / 命令行参考 / API 接口）与上下边框对齐，并新增「命令行参考」区块，列出 `adminctl` / `userctl` / `banip` / `debug` 四组命令及子命令

### v26.09.29

**修复**

- 🐛 MySQL 连接失败时 pymysql 原始堆栈直接冒出让容器崩溃退出，现按错误码分类翻译成中文诊断提示：2003（Connection refused）列出 6 条可操作排查步骤（MySQL 监听、IP/端口、容器网络、防火墙、bind-address、服务启动）；1045（认证失败）提示检查账号密码；超时提示网络不可达；运行期 ping 断开自动丢弃旧连接让下次调用自愈

### v26.09.28

前后端代码整体质量优化，无新增功能。**自本版本起，项目进入休眠维护阶段：不再添加新功能，仅修复缺陷、安全问题与必要的兼容性调整。**

**修复**

- 🐛 `.mux.mp4` 中间源文件大小写判定失效：原代码按大写 `.MUX.mp4` 匹配永远不成立，导致 MKV 输出任务遇到该类源文件时跳过转封装直接改名，产出内容实为 MP4 的 `.mkv` 文件；现统一按小写匹配
- 🐛 并发任务上限存在竞态：任务计数与创建未在同一锁内，并发提交时可能超出 `MAX_CONCURRENT_TASKS`；现改为锁内校验，超限请求返回 429
- 🐛 管理端「过滤模板」弹窗的保存成功 / 正则错误提示因缺失显示样式从未展示过，修复后恢复可见
- 🐛 CLI 返回码与提示修正：`userctl del/ban/unban`、`adminctl del` 对不存在的用户不再误报成功（正确退出码 1 并报错）；`adminctl password` 对普通用户给出正确引导（改用 userctl）；数据库不可用时统一清晰报错退出，不再抛 Python 堆栈
- 🐛 `deploy.sh` 镜像仓库所有者改为小写（Docker 镜像名不允许大写路径，原值会导致构建推送失败）
- 🐛 认证失败警告记录在认证成功后未清理，长时间运行内存缓慢增长，已修复

**变更**

- 🔒 API 内部错误（500）响应不再回传异常细节：详情仅在 debug 模式返回，服务端始终记录日志，避免向调用方泄露内部实现信息
- 🧹 三个网页的重复 JS（转义、提示条、弹层、API 封装、过滤编辑器，约 700 行）收敛到 `shared.js` 的 `window.CD` 统一实现；管理端网络断开提示与登录过期文案随之与用户端统一
- ⌨️ CLI 数据库报错信息按实际后端显示位置（SQLite 显示文件路径，MySQL 显示连接目标），不再固定显示 SQLite 路径
- ⚙️ `API_PORT` 环境变量为非法值时输出警告并回退默认端口（原先静默回退）

### v26.09.27

新增可选 MySQL 数据库后端，默认仍为 SQLite（零依赖）。

**新增**

- 🗄️ 双数据库后端：不设置 `DATABASE_URL` 默认使用 SQLite（`config/data.db`，行为不变）；配置 `DATABASE_URL=mysql://用户:密码@主机:3306/库名` 后切换为 MySQL，目标库需提前建好、账号有建表权限，容器首启自动建表
- 🔐 MySQL 8 认证支持：镜像内置 `python3-cryptography`，兼容 MySQL 8 默认的 `caching_sha2_password` 认证插件；连接空闲被服务端 `wait_timeout` 回收时自动 `ping` 重连

**变更**

- 🐳 Dockerfile apt 安装列表新增 `python3-pymysql` 与 `python3-cryptography`（不引入 pip，镜像仅增约 200KB）
- 📝 docker-compose.yml 与 README 配置示例、环境变量表同步新增 `DATABASE_URL` 注释示例与「数据库后端」小节说明

### v26.09.26

**变更**

- ✂️ 管理网页「命令行参考」卡片描述精简：debug 两条去掉过程日志说明、del 两条去掉括注，详细说明以 README CLI 章节为准
...
- 🐛 容器以非 root 用户运行，entrypoint 运行时无权写 `/usr/local/bin`，别名软链创建失败并因 `set -e` 中止整个启动脚本；别名软链改到**镜像构建阶段**（root 身份）创建，entrypoint 不再涉及
...
- 🔤 命令别名：镜像构建时为四个 CLI 在 `/usr/local/bin` 全局 PATH 创建软链——`adminctl → actl`、`userctl → uctl`、`banip → bip`、`debug → dbg`，两种写法完全等价
...
- 📅 自定义日历：日志导出的起止日期改用随主题配色的自绘日历面板（替代原生日历弹层），未来日期自动禁选，点击面板外 / Esc / 滚动 / 缩放自动关闭

**变更**

- 📐 管理端「服务器配置」与「日志导出」两张卡片宽屏并排显示，手机端上下排列，卡片按钮底部对齐
- ☑️ 「添加为管理员」复选框由表单区移至「添加用户」弹窗标题行右侧

**修复**

- 🕐 日志导出的日期默认值与未来日期校验改按本地时区计算（原先经 UTC 换算，东八区凌晨会把「今天」误算成昨天）

### v26.09.20

管理端新增管理员创建入口与运行日志导出能力。

**新增**

- 👑 管理网页可直接添加管理员：「添加用户」弹窗新增「添加为管理员」选项（管理员的改密/删除仍需容器内 `adminctl`）
- 📝 每日日志：运行日志按天写入 `config/log/YYYY-MM-DD.log`（含全部日志级别，不含 banner），随 config 挂载卷持久化
- 📤 日志导出：管理网页按日期范围把每日日志合并导出为单个文件（单次 ≤15 天、不可选未来日期、无日志日期自动跳过）；新增 `GET /{prefix}/admin/logs/export` 接口

**变更**

- 📖 管理端管理员相关说明文案更新（网页可添加管理员，改密/删除仍需 `adminctl`）
- 🎨 日志导出起止日期输入框样式与全站输入框统一

**修复**

- 🧱 管理端表格「操作」列因 td 直接设置 flex 导致行边框断开，改由内层 div 承载弹性布局

### v26.09.14

首个纳入版本化管理的版本，对早期迭代做一次功能定型。

**新增**

- 🏷️ 版本号体系：以 `web/VERSION` 为唯一来源，启动 banner、登录页、管理端顶栏、用户端服务信息卡片与 `/config` 接口统一展示
- 🖥️ 三套网页控制台（登录 / 下载用户 / 管理员），纯静态外壳 + Bearer 令牌调用 API，登录后按角色自动跳转；旧地址 302 兼容
- 👥 多用户体系与目录隔离：每用户独立的 `user/<名>`（配置/日志）、`temp/<名>`（缓存）、`downloads/<名>`（成品），任务列表按用户过滤；首启自动创建默认管理员 `admin`
- 🎫 两层认证（AUTH_KEY + 用户名/密码）与 2 小时 HMAC 短期令牌，支持登出、改密码、禁用、删用户、更换 AUTH_KEY 等场景的令牌吊销
- 🔄 网页「切换用户」：免 AUTH_KEY 换号，支持用户 ↔ 管理员跨角色
- 🚫 分级封禁：IP 级（10 分钟 10 次 → 临时封 30 分钟 → 再犯永久）+ 账号级（10 分钟 5 次密码错误 → 自动禁用）；`banip` CLI 与管理网页 IP 封禁管理（`/admin/bans` 系列接口）
- 🗂️ 配置分层：系统级配置全部走 docker-compose 环境变量（URL_PREFIX/API_PORT/SSRF_PROTECTION/MAX_CONCURRENT_TASKS/SAME_VIDEO_BY_FILENAME）；AUTH_KEY 存 admin_config.json 并支持管理网页热更新
- 🐳 host 网络模式 + dual-stack 监听，直接获取客户端真实 IPv4/IPv6
- 🎥 下载能力：逐任务 mp4/mkv 输出格式、同视频按文件名聚合轮流调度（5 轮）、断点续传、任务持久化与重启自动恢复
- 📹 直链与嗅探：mp4/mkv/ts/flv/f4v/avi/webm/mov/wmv 直链 curl 下载、`.f4v` 下载后直接改名 `.mp4`（FLV 封装兜底 ffmpeg 转封装）、无扩展名链接 64KB 探测嗅探、CDN JSON 调度链接自动解析（最多 3 跳）
- 🔍 每用户三段过滤规则（拦截关键字 / 文件名过滤 / 正则去重，独立开关）+ 管理员全局模板（新用户自动复制）
- 🐞 `debug yes/no/show` 运行时调试开关（SIGUSR1/SIGUSR2，不写文件、重启恢复关闭）
- 🛡️ SSRF 防护、全接口认证、认证先于限流、1MB 请求体限制、每 IP 速率限制、URL 敏感参数脱敏

**变更**

- 🎨 登录/用户/管理三页样式与代码统一：抽取 `shared.css`/`shared.js` 经占位符注入，删除约 1900 行重复 CSS/JS；管理端补齐主题色与亮暗切换
- 🎨 主题色板改为会话内随机（sessionStorage，三页同色、关闭浏览器后重新随机），亮暗偏好存 localStorage 长期记忆；输出格式下拉框改为自绘组件以适配三色板与暗色模式
- ℹ️ 猫抓发送地址/请求体模板由原弹窗并入下载控制台常驻「服务信息」卡片（同时展示版本号与配置摘要）
- 📝 日志分级：正常模式仅输出关键结果，类型探测/命令执行/任务 ID/路径等细节下沉到 debug 模式
- 🔐 AUTH_KEY 改为仅随机生成（首启约 32 位；管理网页「🎲 随机」生成 14 位），不再支持手工指定弱密钥

**移除**

- 🗑️ 管理网页「重载配置」按钮（`POST /reload` API 保留，供手工改文件后热加载）
- 🗑️ config.json 系统配置文件（系统级配置迁移至 docker-compose 环境变量）
- 🗑️ URL 查询参数传递凭证（`key/user/password` 出现在 URL 中直接返回 403，统一为 Bearer 令牌或请求体）
...
- **🏷️ 版本号展示**：版本号以 `web/VERSION` 单行文本为唯一来源（当前 26.09.28），启动 banner 顶部、登录页与管理端顶栏（均带 `v` 前缀）、下载控制台「服务信息」卡片、`GET /{prefix}/config` 接口均展示同一版本号
- **🎬 完整下载参数**：支持 referer、cookie、user-agent、逐任务输出格式（mp4/mkv）等
- **💾 数据持久化**：配置、用户数据、下载缓存与成品文件均持久化到宿主机，重建容器不丢失
- **🗄️ 双数据库后端**：默认 SQLite（`config/data.db`，零依赖）；设置 `DATABASE_URL=mysql://...` 后切换为 MySQL（目标库需提前建好、账号有建表权限），切换视为全新部署不自动迁移数据
...
- **🛠️ 管理员体系**：管理员/下载用户两种角色；首启自动创建默认管理员 `admin`（14 位随机密码，见启动日志）；管理网页可添加用户/管理员、改密、禁用、热更新 AUTH_KEY、编辑过滤规则模板、导出运行日志；管理员账户的改密与删除通过容器内 `adminctl` 命令
...
- **🚫 分级封禁**：AUTH_KEY 错误或用户不存在按 **IP 级**计数（10 分钟内 10 次 → 临时封禁 30 分钟，再次触发永久封禁）；密码错误按 **账号级**计数（10 分钟内 5 次 → 自动禁用该账号）；`banip` CLI 与管理网页均可管理 IP 封禁
...
- **📝 每日日志与导出**：运行日志按天写入 `config/log/YYYY-MM-DD.log`（含全部级别，不含 banner）；管理网页可按日期范围合并导出为单个日志文件（单次 ≤15 天、不可选未来日期、无日志日期自动跳过）
...
- **🔒 接口认证全覆盖**：网页页面（登录/下载/管理）与 favicon、`/health` 为公开资源（无需认证），其余所有 API 均需认证；未带前缀的请求返回 404
...
- **🎨 三页统一主题**：登录/用户/管理三页共用 `shared.css`/`shared.js`（服务端注入，浏览器收到的仍是完整单文件页面）；三套主题色（墨绿 / 暖橙 / 紫罗兰）在每个浏览器会话首次访问时**随机选定**并存入 sessionStorage（同一会话内三页保持同色，关闭浏览器后重新随机），也可点左上角「🎨 主题色」在三色间循环切换；亮/暗双模式默认亮色，手动切换后存入 localStorage 长期记忆
...
│   ├── VERSION          # 🏷️ 版本号唯一来源（单行纯文本，无扩展名，如 26.09.28）
│   └── favicon.ico      # 🌐 网页图标
├── sh/                  # 📜 Shell 脚本
...
将 `/youdir` 替换为你希望存放下载文件的实际路径。四个目录分别对应：配置与数据库（运行日志按天写入其中的 `log/` 子目录）、每用户配置/日志、下载缓存分片、最终成品视频。
...
2. **启动 banner**：版本号、API 端口/前缀/SSRF/并发/同视频模式/过滤规则摘要、猫抓插件发送地址与请求体模板、API 接口列表
3. **首次启动凭据**（仅首启显示一次）：自动生成的 `AUTH_KEY`（14 位随机字符串，含大小写字母、数字及符号 `!@#$%^&*`）与默认管理员 `admin` 的 14 位随机初始密码（字母+数字），请妥善保存并尽快修改
...
| 容器路径                     | 宿主机路径          | 用途                                                          |
| ---------------------------- | ------------------- | ------------------------------------------------------------- |
| `/home/downloader/config`    | `/youdir/config`    | 系统配置、管理员热配置、全局过滤模板、data.db、每日日志 log/  |
...
| `log/`              | 每日运行日志子目录：`YYYY-MM-DD.log` 按天一个文件，含全部日志级别            |
...
- **任务列表**：每 3 秒自动刷新（递归轮询，同一时刻只有一个请求在途，避免请求堆积），仅显示**当前登录用户自己的任务**；显示文件名、状态徽章（收集中 / 下载中 / 已暂停）、进度条；点击任务可弹出详情（任务 ID、全部链接、重试次数等），可暂停 / 继续 / 删除任务；任务完成（成功或失败）后自动从列表移除
- **外观菜单**（左上角 🐱 CatDock）：「🎨 主题色」在墨绿/暖橙/紫罗兰间循环切换（仅当前会话有效）、「🌙 亮暗切换」（手动选择后长期记忆）；顶栏还有实时健康状态指示
- **账号菜单**（右上角）：修改密码（旧密码 1 遍 + 新密码 2 遍，成功后全部令牌吊销并回到登录页）、切换用户（输入目标账号用户名+密码即可换号，无需重新输入 AUTH_KEY，支持用户 ↔ 管理员跨角色，成功后按角色自动跳转）、过滤规则编辑、退出（红色）
- **服务信息卡片**（页面底部常驻）：展示版本号与运行配置摘要（端口/前缀/SSRF/并发/同视频模式/过滤开关/下载用户数），以及当前前缀对应的猫抓发送地址、请求体模板（含 `key`/`user`/`password`/`format` 占位）与常用 API 接口一览
...
- **用户管理**：查看全部用户（用户名/角色/状态/创建时间）、添加下载用户或管理员（「添加用户」弹窗标题行勾选「添加为管理员」）、删除用户（确认弹窗明确提示配置目录、下载缓存、成品文件将全部删除且不可恢复）、重置用户密码、禁用（ban）/解禁（unban）用户；被禁用用户立即无法登录且已签发令牌全部失效，禁用状态下即使密码正确也返回 403 且不计入 IP 封禁
...
- **日志导出**：在「日志导出」卡片选择起止日期，把范围内的每日日志合并导出为单个文件（单次 ≤15 天、不可选未来日期、无日志日期自动跳过）；日期通过随主题配色的自绘日历面板选择（未来日期禁选，点击面板外 / Esc / 滚动自动关闭）；宽屏下「服务器配置」与「日志导出」两张卡片并排，手机端上下排列
- **顶栏菜单**：左上角外观菜单（🎨 主题色 / 🌙 亮暗切换，与登录页、下载控制台同一套机制），顶栏显示 `v 版本号`；右上角账号菜单含修改密码、切换用户（可跨角色切到下载用户）、过滤模板、退出
- **修改密码**：管理员修改自己的密码（同账号菜单「修改密码」）
- **命令行参考**：页面内附 `adminctl` / `userctl` / `banip` / `debug` 全部命令速查（桌面端 3 列并排，平板 2×2，手机单列）
- **管理员账户改密/删除**：管理网页仅支持添加管理员，改密与删除仅通过容器内 `adminctl` CLI 完成（见命令行工具章节），至少保留一个管理员

## 📡 API 接口

> 注意：**所有接口路径均需要添加 URL 前缀**（如 `/qj52lajx`），未带前缀的请求返回 `404 Not Found`。网页页面（`/login.html`、`/user.html`、`/admin.html` 三个规范地址，旧地址 302 跳转）与 `favicon.ico`、`/health` 为公开资源（无需认证）；其余 API 均需认证，**推荐使用访问令牌（Bearer Token）**：先调用 `POST /{prefix}/login` 用 `key`/`user`/`password`（请求体）换取 2 小时有效期的短期令牌，后续请求在 `Authorization: Bearer <token>` 请求头中携带令牌即可。**凭证不通过 URL 查询参数传递**（URL 会进入浏览器历史与各级访问日志，存在泄露风险）。POST 接口同时兼容请求体传 `key`/`user`/`password`（如猫抓插件等第三方调用方）。
...
| `/{prefix}/config`               | GET      | 获取运行配置摘要（版本号/端口/前缀/开关/下载用户数，AUTH_KEY 脱敏）                                                   | 令牌          |
...
| `/{prefix}/admin/users/add`      | POST     | 添加下载用户或管理员（`role` 传 `admin` 时创建管理员）                                                                | 管理员        |
...
| `/{prefix}/admin/bans`           | GET      | 获取封禁 IP 列表（IP/封禁时间/原因）                                                                                  | 管理员        |
| `/{prefix}/admin/bans/add`       | POST     | 手动封禁 IP（校验 IPv4/IPv6 合法性，永久封禁）                                                                        | 管理员        |
| `/{prefix}/admin/bans/delete`    | POST     | 解封 IP（同时清除失败计数与阶梯封禁统计）                                                                             | 管理员        |
| `/{prefix}/admin/logs/export`    | GET      | 导出日志（`?start=YYYY-MM-DD&end=YYYY-MM-DD`，单次 ≤15 天、不可选未来日期、无日志日期自动跳过，合并为单个 .log 下载） | 管理员        |
...
> **吊销机制**：令牌签名依赖 AUTH_KEY 且签发时登记唯一 `jti` 到 `auth_tokens` 表，每次业务请求校验签名与吊销状态。除网页「退出」外，以下操作也会批量作废令牌：`userctl password` 修改密码、禁用用户（`userctl ban` / 管理网页）、删除用户（防止重建同名用户后旧令牌复活）——该用户全部令牌失效；管理网页热更新 AUTH_KEY 或管理员执行 `/reload` 导致 KEY 变化——**全员**令牌失效（签名同时失效，双保险）。已过期的登记行会在下次签发令牌时自动清理。
...
重新加载配置文件，无需重启容器。**仅管理员可用**（携带管理员令牌调用；管理网页无入口）：
...
| `DATABASE_URL`           | 数据库后端：不设置用 SQLite；`mysql://用户:密码@主机:3306/库名` 切换为 MySQL（切换视为全新部署，不自动迁移数据） | 空（SQLite）                             | 否       |
...
默认使用 **SQLite**（`config/data.db`，零外部依赖），开箱即用。如需集中管理或与其他服务共享数据库，可切换为 **MySQL**：

1. **提前建库**：在目标 MySQL 实例上创建空数据库与账号（账号需有建表权限，容器首启自动建表）：

   ```sql
   CREATE DATABASE catdock CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
   CREATE USER 'catdock'@'%' IDENTIFIED BY '你的密码';
   GRANT ALL PRIVILEGES ON catdock.* TO 'catdock'@'%';
   FLUSH PRIVILEGES;
   ```
...
> **切换视为全新部署**：从 SQLite 切到 MySQL（或反之）不自动迁移数据，MySQL 首启会走"创建默认管理员 admin + 14 位随机密码（见启动日志）"流程；旧 SQLite 里的用户/封禁/令牌不会带过去。需要迁移请手工导出导入。
>
> **MySQL 8 认证**：MySQL 8 默认认证插件为 `caching_sha2_password`，镜像已内置 `python3-cryptography` 支持；若 MySQL 侧改回 `mysql_native_password` 也可，二者皆可连通。
>
> **连接自愈**：MySQL 服务端 `wait_timeout` 会回收空闲连接，驱动在每次访问前自动 `ping` 重连，长期运行无需额外维护。
...
1. **下载历史**：检查该用户 `success.log` 和 `failure.log`（首次使用时全量读入内存缓存，之后随下载增量更新）
2. **任务列表**：检查当前正在运行的任务
...
> **命令别名**：镜像构建时在 `/usr/local/bin`（全局 PATH）创建别名软链，与原命令两种写法完全等价。
>
> | 原名       | 别名   |
> | ---------- | ------ |
> | `adminctl` | `actl` |
...
- **令牌方式（Bearer）**：确认令牌未过期（2 小时有效期）且未被吊销（退出/改密码/删用户/禁用用户/更换 AUTH_KEY 后全部失效），过期或失效后重新调用 `POST /{prefix}/login` 获取
...
- 确认宿主机 `API_PORT` 端口（docker-compose.yml 示例为 5000，未设置环境变量时默认 8080）未被占用且防火墙已放行
...
4. **配置修改**：每用户过滤规则在网页「过滤规则」中编辑后立即生效，无需重载；管理员在管理网页保存全局过滤模板（`config/filter_rules.json`）也即时生效；若手工直接改了宿主机上的 `admin_config.json` / `filter_rules.json`，可调用 `POST /{prefix}/reload` 接口热加载（auth_key 也可直接在管理网页「服务器配置」修改，立即生效并全员重新登录）；系统级开关（docker-compose 环境变量）不随 reload 生效，修改后需 `docker-compose up -d` 重建容器
...
A: `AUTH_KEY` 保存在 admin_config.json 的 `auth_key` 字段：首次启动若为空或 `CHANGE_ME` 会自动随机生成（14 位，含大小写字母、数字及符号 `!@#$%^&*`）并写回文件，同时在启动日志打印一次。修改方式：管理员登录管理网页 `/{prefix}/admin.html`，在「服务器配置」中点「🎲 随机」生成新 KEY（输入框只读，生成的是 14 位、含大小写字母+数字+符号的随机串）并保存，立即写回 admin_config.json 并生效——**包括管理员在内全员令牌吊销**，所有用户用新 KEY 重新登录即可，用户密码无需改动、无需重启容器。`URL_PREFIX` 仍仅通过环境变量配置以保证访问地址恒定。用户密码（第二层认证）通过 `userctl add/password` 或管理网页管理；管理员账户通过 `adminctl` CLI 管理。

**Q: 默认管理员账户是什么？**
A: 首次启动自动创建管理员 `admin`，密码为 14 位随机字符串，仅在启动日志显示一次（`docker logs catdock`）。请妥善保存并尽快登录管理网页修改；管理员登录后自动进入管理控制台，可添加用户/管理员、删除下载用户、重置密码、禁用用户、热更新 AUTH_KEY、编辑全局过滤模板、导出日志。管理员账户的改密与删除需在容器内用 `adminctl password/del` 命令操作。
