...
> **当前版本：v26.09.26**（版本号以 [`web/VERSION`](web/VERSION) 为唯一来源；各版本变更见文末「[更新日志](#-更新日志)」）
...
- **🏷️ 版本号展示**：版本号以 `web/VERSION` 单行文本为唯一来源（当前 26.09.26），启动 banner 顶部、登录页与管理端顶栏（均带 `v` 前缀）、下载控制台「服务信息」卡片、`GET /{prefix}/config` 接口均展示同一版本号
...
│   ├── VERSION          # 🏷️ 版本号唯一来源（单行纯文本，无扩展名，如 26.09.26）
│   └── favicon.ico      # 🌐 网页图标
├── sh/                  # 📜 Shell 脚本
