...
- **📹 直接视频下载**：支持 mp4、mkv、ts、flv、f4v、avi、webm、mov、wmv 等格式的直链下载（curl 断点续传）；`.f4v` 下载后自动用 ffmpeg 无损转封装为所选 mp4/mkv
- **🧭 CDN 调度链接自动解析**：部分站点（如爱奇艺 `pcw-data.video.iqiyi.com`）的媒体链接返回 JSON 调度响应而非视频本身，自动解析其中的真实 CDN 地址并跟随下载（经 SSRF 校验，最多 3 跳）
...
- 支持直接视频链接（mp4、mkv、ts、flv、f4v、avi、webm、mov、wmv 等），自动使用 curl 下载；`.f4v` 下载完成后会自动无损转封装为所选输出格式（mp4/mkv），爱奇艺等站点的 CDN 调度链接（返回 JSON 而非视频）也会自动解析真实地址后下载，详见[直链视频下载](#直链视频下载与-f4v-支持)
...
| 参数        | 类型   | 必填 | 默认值              | 说明                                                                                                                                                        |
| ----------- | ------ | ---- | ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `url`       | string | 是   | —                   | m3u8 视频链接或直接视频链接（mp4/mkv/ts/flv/f4v/avi/webm/mov/wmv 等直链自动用 curl；`.f4v` 自动转封装为所选格式；返回 JSON 调度响应的链接自动解析真实地址） |
...
5. 检查是否存在未转换的源文件（`.ts` / `.mux.mp4` / 直链下载后未转封装的 `.f4v`）→ 自动补转换/补转封装
...
URL 路径以视频扩展名结尾的链接视为**直链视频**，使用 curl 下载（`-L -C -` 跟随跳转、断点续传），支持以下扩展名：
...
- **`.f4v` 直链**：`.f4v` 是 Adobe 碎片化 MP4 容器的扩展名，但爱奇艺等站点的 `.f4v` 链接实际内容多为 **FLV 封装**（文件头 `FLV`），保留 `.f4v` 扩展名多数播放器无法识别。因此下载完成后会自动调用 ffmpeg **无损转封装**（`-c copy`，不重新编码，秒级完成）为任务选定的输出格式（mp4/mkv），并删除中间 `.f4v` 文件。
  - 转封装最多尝试 3 次；若因编码不被目标容器支持等原因失败，兜底将源文件改名为 `.flv` 保留（内容通常为 FLV），不会因转封装失败丢失已下载数据。
  - 容器重启/重建后发现残留的未转封装 `.f4v` 源文件时，会自动补做转封装。
- **CDN 调度链接自动解析**：部分站点（典型为爱奇艺 `pcw-data.video.iqiyi.com`）的媒体链接返回的不是视频本身，而是一小段 JSON 调度响应：

  ```json
  { "l": "https://真实CDN节点/xxx.f4v?...", "e": "0" }
  ```

  下载完成后若发现文件过小（< 64KB）且内容为 JSON，会自动解析 `l`（兼容 `url`）字段中的真实 CDN 地址，删除小文件后用真实地址重新下载：
  - 真实地址必须通过 SSRF 安全校验（禁止指向内网/回环地址）；
  - 最多跟随 3 跳，防止调度循环；切换地址**不消耗**下载重试次数；
  - 任务记录与 success/failure 日志始终保存用户提交的**原始调度链接**（稳定、可去重），重启恢复时重新解析获取最新 CDN 节点。
